...
namespace Yarn.Unity.Kostas
{
    ...
    /// elements.
    /// </summary>
    public class LineViewKostas : DialogueViewBase
    {
        public Slider progressBar;
        public TMP_Text progressText;
        float maxDialogueLines = 86f;
        ...
        [SerializeField]
        internal List<CanvasGroup> canvasGroup;
        ...
        /// </summary>
        [SerializeField]
        internal List<TextMeshProUGUI> lineText = null;
        ...
        /// next options) until all Dialogue Views have reported that they haveS
        ...
        private void Reset()
        {
            for (int i = 0; i < 2; i++)
            {
                canvasGroup[i] = GetComponentInParent<CanvasGroup>();
            }
        }
        ...
        {
            string temp_char_name = currentLine.CharacterName;
            currentLine = null;

            StartCoroutine(DismissLineInternal(onDismissalComplete,temp_char_name));
        }

        private IEnumerator DismissLineInternal(Action onDismissalComplete, string name)
        {
            // disabling interaction temporarily while dismissing the line
            // we don't want people to interrupt a dismissal
            int i;
            if (name.Equals("Σωκράτης"))
            {
                i = 0;
            }
            else
            ...
            var interactable = canvasGroup[i].interactable;
            canvasGroup[i].interactable = false;

            // If we're using a fade effect, run it, and wait for it to finish.
            if (useFadeEffect)
            {
                yield return StartCoroutine(Effects.FadeAlpha(canvasGroup[i], 1, 0, fadeOutTime, currentStopToken));
        ...
        public override void InterruptLine(LocalizedLine dialogueLine, Action onInterruptLineFinished)
        {
            ...
            int i;
            if (dialogueLine.CharacterName.Equals("Σωκράτης"))
            {
                i = 0;
            }
            else
            ...
            lineText[i].gameObject.SetActive(true);
            canvasGroup[i].gameObject.SetActive(true);
            ...
            if (characterNameText == null)
            {
                ...
                {
                    lineText[i].text = dialogueLine.Text.Text;
                    length = dialogueLine.Text.Text.Length;
                }
                else
                {
                    lineText[i].text = dialogueLine.TextWithoutCharacterName.Text;
        ...
        private IEnumerator RunLineInternal(LocalizedLine dialogueLine, Action onDialogueLineFinished)
        {
            ...
            IEnumerator PresentLine()
            {
                ...
                if (useTypewriterEffect)
                {
                    ...
                    // in on visible text).
                    lineText[i].maxVisibleCharacters = 0;
                }
                else
                {
                    // Ensure that the max visible characters is effectively
                    // unlimited.
                    lineText[i].maxVisibleCharacters = int.MaxValue;
                }
                ...
                if (useFadeEffect)
                {
                    yield return StartCoroutine(Effects.FadeAlpha(canvasGroup[i], 0, 1, fadeInTime, currentStopToken));
            ...
            //Here we will invoke animation
            //if(i==1)
            //{
            //    TalkAnimate.SetTrigger("IsTalking");
            //}
        ...
        public void OnContinueClicked()
        {
            ...
            progressBar.value += (1f / maxDialogueLines);
            progressText.text = String.Format("{0:0.00}", (progressBar.value * 100f)) + "%";
